...
export default function Trash() {
  ...
  React.useEffect(() => {
    const getAcc = async () => {
        ...
        const get_api = await axios.get(process.env.REACT_APP_API_URL+'/trash_list/'+accounts[0]+'');
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 2 }}>
        ...
        {req_data.map((row,index) => 
            <ListItem button component="a" href={'/Detail_extra?id='+row['id']} key={row['id']} className={'bdr_btm_1 bk_clr_'+row['status']}>
                <ListItemAvatar>
                <Avatar>
                    <ImageIcon />
                </Avatar>
